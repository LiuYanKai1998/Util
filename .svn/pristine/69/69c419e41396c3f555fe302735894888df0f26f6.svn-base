package ext.wisplm.apiexercise.jcratecadcadlink;
...
public class Link implements RemoteAccess{
	...
	public static void main(String args[]) throws RemoteException, InvocationTargetException{
		String parentEPMNumber 		     = args[0];
		String childEPMNumber 	     	 = args[1];
		RemoteMethodServer rms = RemoteMethodServer.getDefault();
		Class cla[] = {String.class,String.class};
		Object obj[] = {parentEPMNumber,childEPMNumber};
		rms.invoke("createEpmToEpmLink", Link.class.getName(), null, cla, obj);
		return;
	}
...
/*        if (WorkInProgressHelper.isCheckedOut(parent)){
	 ...
	 * 为部件创建说明或者参考文档
	 */
	public static boolean createPartDocRelation(WTPart part, WTDocument doc)
			throws WTException {
		WTList newLinks = new WTArrayList();
		...
		if (isReference) {
			...
			for (Iterator it = map.keySet().iterator(); it.hasNext();) {
				...
				if (oldDocument.getNumber().equals(doc.getNumber())) {
					...
					WTPartDescribeLink link = (WTPartDescribeLink) map
							.get(oldDocument);
